...
for filename in os.listdir('../../images/'):
     img = cv2.imread('../../images/Batteries' + filename, cv2.IMREAD_COLOR)
...
resultarray = [0, 0, 0]
for filename in os.listdir('../../images/'):
     img = cv2.imread('../../images/Cans' + filename, cv2.IMREAD_COLOR)
     test_image = cv2.resize(img, (150, 150), interpolation=cv2.INTER_AREA)
     test_image = test_image / 255
     test_image = np.expand_dims(test_image, axis=0)
     result = classifier.predict_proba(test_image)
...
resultarray = [0, 0, 0]
for filename in os.listdir('../../images/'):
     img = cv2.imread('../../images/Glas' + filename, cv2.IMREAD_COLOR)
     test_image = cv2.resize(img, (150, 150), interpolation=cv2.INTER_AREA)
     test_image = test_image / 255
     test_image = np.expand_dims(test_image, axis=0)
     result = classifier.predict_proba(test_image)
