...
from keras.layers import Convolution2D, Dropout, LeakyReLU, Conv2D
...
graph_acc_name = "cnn_29_acc"
graph_loss_name = "cnn_29_loss"
model_name = "cnn_29.h5"
kernel = 3
stride = 1
image_size_x = 150
image_size_y = 150
...
classifier.add(Conv2D(filters=128, kernel_size=kernel, strides=stride, padding='same'))
classifier.add(LeakyReLU(alpha=0.1))
classifier.add(get_maxpool_layer(2))

classifier.add(Conv2D(filters=256, kernel_size=kernel, strides=stride, padding='same'))
classifier.add(LeakyReLU(alpha=0.1))
classifier.add(get_maxpool_layer(2))

classifier.add(Conv2D(filters=512, kernel_size=kernel, strides=stride, padding='same'))
classifier.add(LeakyReLU(alpha=0.1))
classifier.add(get_maxpool_layer(2))
